...
#
# Quick crosswalk:
#   node_level_long.ChildID     -> ego_level_network_summary.ChildID   (many-to-one)
#   activity_level_long.ChildID -> ego_level_network_summary.ChildID   (many-to-one)
#
# Naming conventions:
...
# =============================================================================
# node_level_long  — Alter-level table (relationships)
# =============================================================================
# 1) What is this?
...
#    Foreign key: ChildID -> ego_level_network_summary.ChildID
...
#
#    -- ANALYTICS
#       (component_membership removed - no longer necessary)
...
# =============================================================================
# ego_level_network_summary  — Child-level table (ego)
# =============================================================================
# 1) What is this?
...
#    Grain: ChildID
#    Primary key: ChildID
#    Receives aggregates from node_level_long.

# 3) Must-have columns
#    ChildID, survey_enddate, survey_completion

# 4) Field map (grouped)
#    -- IDs & META
#       ChildID, child_name (PII), survey_enddate (MM/DD/YYYY), survey_completion
#
#    -- CHILD DEMOGRAPHICS
#       child_birthdate, child_age_in_months
#       child_gender
...
#       network_size, network_density, network_edges_number
#       network_component_count, network_component_ratio
#       network_75cutoff_validity ("yes" if ≥75% alters have demo fields else "rejected")
#
#    -- DIVERSITY / EI
...
#    • survey_enddate accepts Qualtrics serials and ISO strings; formatted MM/DD/YYYY.
#    • network_component_ratio = (network_component_count - 1) / (network_size - 1), NA if size<=1.

# =============================================================================
# activity_level_long  — Activity-level table
# =============================================================================
# 1) What is this?
...
#    • node_level_long rows that originate from activities copy the label into
...
# Demographics loading into ego_level_network_summary
...
# Node processing into node_level_long
...
ego_fields <- function(ego_level_network_summary) {
  list(
    ...
    ego_race          = {
      main <- na_blank(pull_safe(ego_level_network_summary, "child_race"))
      txt  <- na_blank(pull_safe(ego_level_network_summary, "child_race_6_TEXT"))
...
# Alias to new canonical name for clarity without breaking downstream references
node_level_long <- node_level_long

# =============================================================================
# 4) Build activity_level_long (keeps ChildID) — unchanged logic, minor safety
# =============================================================================
...
# prefer a human-readable ChildID; fall back sensibly
resolve_child_id <- function(d, prefer = c("ChildID"),
                             fallback_prefix = "R") {
...
# Prepare df (wide) and explicitly carry ChildID
...
df1 <- df %>%
  mutate(
    ...
    # This guarantees a ChildID column exists even if the raw df lacks it
    ChildID = resolve_child_id(.)
  )
...
# Pivot ONLY activity fields — keep ChildID through the pivot
activity_level_long_pre <- df1 %>%
  select(.rowid, .join_key, ChildID, all_of(all_act_cols)) %>%
  pivot_longer(
...
# Pivot Activity_loading_* (no ChildID here; we already have it)
loading_long <- df1 %>%
...
} else if ("ChildID" %in% names(node_level_long) && "ChildID" %in% names(df)) {
  join_vec <- as.character(node_level_long[["ChildID"]])
} else {
  join_vec <- rep("1", nrow(node_level_long))
  warning("No common join key found between df and node_level_long; activity counts are global (not per respondent).")
}
...
# Final activity_level_long with ChildID kept
...
# Assumptions:
#  - node_level_long and ego_level_network_summary already exist (previous pipeline steps).
...
#  - All network metrics are calculated regardless of validity status.
#  - Users can filter based on network_75cutoff_validity column if desired.
# We store validity per ChildID in demo_flags, and join into ego_level_network_summary.
# =============================================================================
...
race_entropy <- node_level_long %>%
  ...
  summarise(
    network_racial_entropy = -sum(p * log2(p)),
...
racial_ei_by_child <- ei_work %>%
  ...
  summarise(
    ...
    network_racial_ei_index = if_else(valid > 0, (network_racial_outgroup - network_racial_ingroup) / valid, NA_real_),
    .groups = "drop"
  ) %>%
...
language_entropy_by_child <- lang_prop %>%
  ...
  summarise(
    network_language_entropy  = entropy_bits(prop),
...
linguistic_ei_by_child <- lang_ei_work %>%
  ...
  summarise(
    ...
    network_linguistic_ei_index = if_else(
      ...
      (network_lang_outgroup - network_lang_ingroup) / network_size_valid_lang,
      NA_real_
    ),
    .groups = "drop"
  ) %>%
...
compute_density_per_child <- function(df_child) {
  ...
  tibble(network_density = dens, network_edges_number = network_edges_number)
}
...
# =============================================================================
# Note: All values are calculated regardless of validity status
# Users can filter based on network_75cutoff_validity column if desired
# =============================================================================
...
cols_child_demo <- c(
  "child_birthdate", "child_age_in_months", "child_gender", "child_race", "child_race_detail",
  "child_zipcode"
)
...
write.csv(node_level_long,              '/Users/beiniou/Desktop/SNQ Pipeline/sample_output/node_level_long.csv',              row.names = FALSE)
write.csv(ego_level_network_summary,    '/Users/beiniou/Desktop/SNQ Pipeline/sample_output/ego_level_network_summary.csv',    row.names = FALSE)
write.csv(activity_level_long,          '/Users/beiniou/Desktop/SNQ Pipeline/sample_output/activity_level_long.csv',          row.names = FALSE)
